...
func getOrganizationArtifactCreds(ctx context.Context, organization *models.Organization, store *dmstores.DMStore) *models.OCILoginCreds {
...
func getOrganizationDPK8sInfra(ctx context.Context, organization *models.Organization, infraId string, store *dmstores.DMStore) (*models.K8SInfraParams, error) {
...
func getOrganizationK8SConfig(ctx context.Context, organization *models.Organization, infraId string, store *dmstores.DMStore) (*api.Config, string) {
...
func GetSearchEmbeddings(ctx context.Context, content string, ctxClaim map[string]string, dmStore *dmstores.DMStore, logger zerolog.Logger) ([]float32, error) {
	aiModelParams := dmstores.GetAccountAIAttributes(ctx, dmStore, ctxClaim)
	if aiModelParams != nil {
		return pkgs.VapusSvcInternalClientManager.GenerateEmbeddingsClient(ctx, content, types.DataSourceMetadataVectorDimensions, aiModelParams, logger)
	}
	return nil, apperr.ErrNoAIModelParams
}
