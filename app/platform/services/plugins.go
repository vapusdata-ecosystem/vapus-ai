...
import (
	...
	"github.com/vapusdata-ecosystem/vapusdata/platform/utils"
)
...
func (s *DMServices) NewPluginManagerAgent(ctx context.Context, opts ...PluginIntAgentOpts) (*PluginManagerAgent, error) {
...
func (v *PluginManagerAgent) configurePlugin(ctx context.Context) error {
	...
	err = v.ConfigurePlugin(ctx, plugin, v.CtxClaim)
	if err != nil {
...
func (v *PluginManagerAgent) patchPlugin(ctx context.Context) error {
	existingObj, err := v.GetPlugin(ctx, v.managerRequest.GetSpec().PluginId, v.CtxClaim)
	if err != nil {
	...
	err = v.PutPlugin(ctx, existingObj, v.CtxClaim)
	if err != nil {
		v.Logger.Error().Err(err).Msg("error while configuring plugin")
...
func (v *PluginManagerAgent) describePlugin(ctx context.Context) error {
	result, err := v.GetPlugin(ctx, v.getterRequest.GetPluginId(), v.CtxClaim)
	if err != nil {
		v.Logger.Error().Err(err).Msg("error while getting plugin from datastore")
		return dmerrors.DMError(apperr.ErrPlugin404, err)
	}
...
func (v *PluginManagerAgent) archivePlugins(ctx context.Context) error {
	...
	err = v.PutPlugin(ctx, result, v.CtxClaim)
	v.result = nil
	return nil
}
