...
func (dms *AIStudioServices) AccessTokenAgentHandler(ctx context.Context, request *pb.AccessTokenInterfaceRequest) (*pb.AccessTokenResponse, error) {
...
func (dms *AIStudioServices) SignupHandler(ctx context.Context, request *pb.RegisterUserRequest) (*pb.AccessTokenResponse, error) {
	validTill := time.Now().Add(utils.DEFAULT_PLATFORM_AT_VALIDITY)
...
func (dms *AIStudioServices) GeneratePlatformAccessToken(ctx context.Context, idToken, refreshToken, organization string, ctxClaim map[string]string) (string, mpb.AccessTokenScope, error) {
