...
export function addTable(canvas, tableId) {
  try {
    ...
    responseTableSet.classList.add(
      "mb-1",
      "text-gray-100",
      "rounded-lg",
      "border-zinc-500",
...
export function addTableBody(table) {
  try {
    ...
    responseTableBody.classList.add(
      "bg-zinc-800",
      "divide-y",
      "divide-zinc-500",
      "break-words",
...
export async function buildAndDownloadFile(
  file,
  fileDescription,
  canvas,
  showContent = false,
...
export async function fetchAndRenderNabhikFile(
  file,
  fileDescription,
  container
) {
  try {
    if (
      ...
      renderNabhikFiles(file, fileDescription, container, datadiv.id);
