...
const CreateModelsNodes = () => {
  ...
  // Dummy domain data - replace with actual data from API or props
  ...
  // Fetch enums data and service provider logos
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Combined form data with networkParams:", combinedFormData);
    ...
    // Fix guardrails structure for API payload - extract only IDs
  ...
  const submitForm = async (payload) => {
    try {
      ...
      // API endpoint call
      const output = await modelsRegistryFormApi.getmodelsRegistryForm(payload);
      ...
      if (resourceInfo) {
        ...
        setTimeout(() => {
          router.push(`/ai-center/models-registry`);
        }, 1000);
      } else {
                    ...
                    <fieldset className="p-4 rounded">
                      ...
                      <fieldset className="rounded mb-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {/* Service Provider - Enhanced with images */}
...
export default CreateModelsNodes;
