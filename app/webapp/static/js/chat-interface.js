var box̧BgColor = "bg-[#1b1b1b]";

var dataPointerSvg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="h-6 w-6"
  ...
  export function addUserInputBox(inputElementId,content, canvas) {
    let seperator = document.createElement("div");
    seperator.style.margin = "4px 0";
    seperator.classList.add("text-right");
    seperator.innerHTML = `<div class="bg-zinc-500 font-semibold break-words px-4 py-2 rounded-lg mt-3 mb-2 w-auto h-auto">
                            <p class="text-sm text-gray-100"> ${content}</p>
                        </div>
                        <div class="flex justify-end">
                          <button class="flex items-center pl-1 text-neutral-60 opacity-90 transition-opacity hover:text-neutral-70 group-hover:opacity-100 cursor-pointer" title="Copy to clipboard" onclick="copyToClipboard('${content}')">
                            <svg viewBox="0 0 24 24" width="1.2em" height="1.2em" class="h-4 w-4 rounded-sm p-0.5 hover:bg-neutral-10.5 text-gray-100">
                          ...
                          <button class="flex items-center pl-1 text-neutral-60 opacity-90 transition-opacity hover:text-neutral-70 group-hover:opacity-100 cursor-pointer" title="Retry this message" onclick="retryInput('${content}','${inputElementId}')">
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" class="h-4 w-4 rounded-sm p-0.5 hover:bg-neutral-10.5 text-gray-100">
...
export function addchatDataErrBox(cont, canvas) {
    let obj = {};
    try {
        obj = JSON.parse(cont);
    } catch (error) {
    ...
    let content = `
        <div class="mt-2 w-full p-1 shadow-md rounded-lg ${box̧BgColor}">
            <div class=" flex min-h-16 justify-start">
              <div class="flex grow items-left justify-between overflow-hidden p-2 ${box̧BgColor}">
                <div class="flex w-full gap-4">
                    <svg viewBox="0 0 24 24" class="h-8 w-8" fill="red" stroke="red">
                        <path fill="red" d="M11 9h2V7h-2m1 13c-4.41 0-8-3.59-8-8s3.59-8 8-8s8 3.59 8 8s-3.59 8-8 8m0-18A10 10 0 0 0 2 12a10 10 0 0 0 10 10a10 10 0 0 0 10-10A10 10 0 0 0 12 2m-1 15h2v-6h-2z">
                        </path>
                    </svg>
                  <div class="flex w-full flex-col text-red-400">
...
export function addchatDataSuccessBox(cont, canvas) {
    let obj = {};
    try {
        obj = JSON.parse(cont);
    } catch (error) {
    ...
    var content = `
        <div class="mt-2 w-full p-1 shadow-md rounded-lg ${box̧BgColor}">
            <div class="text-primary-black flex min-h-16 justify-start">
              <div class="flex grow items-left justify-between overflow-hidden p-1 ${box̧BgColor}">
                <div class="flex w-full gap-4">
                  ...
                  <div class="flex w-full flex-col text-green-500">
                      <p class="text-sm font-bold items-left">
                          ${obj.key}
                      </p>
                      <p class="text-xs font-medium">
...
export function addchatDataBox(title, content, canvas,finalSuccess) {
  const box = document.createElement('div');
  box.classList.add('box', "flex", "mb-1", "h-auto", "items-center",  "p-1", "break-words", "text-gray-100",box̧BgColor);
  let fContent = "";
  try {
  ...
  console.log("content --------->>>>>>>>>>>>>>>>>>>>>>>>>>",fContent);
  if (finalSuccess) {
      box.classList.remove("border-l-2");
      box.innerHTML = `
          ${stepSvg}
          <span class="p-1 text-xs text-gray-100 break-words">
              ${renderMDContent(fContent)}
          </span>`;
  } else {
    if (title !== null) {
      title = title.trim();
      if (title !== "" || title.toLowerCase() !== "info") {
        ...
        box.innerHTML = `
              ${stepSvg}
              <span class="p-1 ml-2 text-xs w-auto  text-gray-100  break-words">
                   ${renderMDContent(fContent)}
              </span>`;
      }
    } else {
...
export function addchatCodeBox(content, canvas) {
  var fContent = "";
  try {
      var json = JSON.parse(content);
      var contentVals = Object.values(json)
  ...
  const box = document.createElement('div');
  box.classList.add('box', "flex", "mb-1", "h-auto", "items-center",  "p-1", "break-words", "text-gray-100");
  
  var code = document.createElement('code');
  code.classList.add('text-xs', 'text-gray-100', 'font-semibold', 'break-words');
...
export function addChatDateTimeBox(title, content, canvas) {
  const box = document.createElement('div');
  box.classList.add('box', "flex", "mb-1", "h-auto", "b-4/6", "items-center",  "p-1", "break-words", "text-gray-100", "gap-2");
  box.classList.remove("border-l-2");
  var datetime = document.createElement('span');
  datetime.classList.add('text-xs', 'text-blue-500', 'font-semibold', 'break-words');
  datetime.innerHTML = title + " " + formatUnixTime(parseInt(content));
  box.appendChild(datetime);
  canvas.appendChild(box);
  canvas.scrollTop = canvas.scrollHeight;
}

export function addChatReasoningDataBox(data, canvas) {
  var content = data;
  try {
  ...
  const box = document.createElement('div');
  box.classList.add('box', "flex", "mb-1", "h-auto", "b-4/6", "items-center", "p-1", "break-words", "border-l-2", "text-gray-100", "gap-4");
  box.classList.remove("border-l-2");
  box.innerHTML = `
          <span>
                <svg xmlns="http://www.w3.org/2000/svg" fill="yellow" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" title="Thoughts">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 3c4.97 0 9 4.03 9 9 0 4.97-4.03 9-9 9a9 9 0 0 1-8-13.38L3 3l5.62 1A8.97 8.97 0 0 1 12 3z" />
                  <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01" />
              </svg>

          </span>`
  var reasonings = document.createElement('ul');
  reasonings.classList.add('list-decimal', 'list-inside', 'text-xs', 'text-gray-100', 'break-words');
  content.forEach(function (reason) {
    var li = document.createElement('li');
    li.innerHTML = reason;
    reasonings.appendChild(li);
  });
  box.appendChild(reasonings);
  canvas.appendChild(box);
  canvas.scrollTop = canvas.scrollHeight;
}


export function addTable(canvas, tableId) {
  try {
      let responseTableSet = document.createElement('table');
      responseTableSet.classList.add("mb-1","text-gray-100", "rounded-lg","border-zinc-500", "shadow-md", "border", "divide-y", "divide-zinc-500","w-4/5","ds_table");
      canvas.appendChild(responseTableSet);
...
export function addTableBody(table) {
  try {
      let responseTableBody = document.createElement('tbody');
      responseTableBody.classList.add("bg-zinc-800", "divide-y", "divide-zinc-500", "break-words", "text-xs");
...
export function addTableHeader(table, columns) {
  try {
      const headerElem = document.createElement('thead');
      headerElem.classList.add("bg-zinc-700", "text-sm", "text-gray-100","w-[90%]");
      let headerRow = document.createElement('tr');
      columns.forEach(column => {
          let headerCell = document.createElement('th');
          headerCell.innerHTML = column;
          headerCell.classList.add( "text-center", "border-b", "border-zinc-500");
...
export async function buildAndDownloadFile(file,fileDescription, canvas, showContent = false,download = false) {
    // Determine the MIME type based on the format
    var format = file.format.toLowerCase();
    ...
    if (download) {
        ...
        return fetchAndRenderNabhikFile(file,fileDescription, canvas);
    } else {
        console.log("File Handled");
    }
}

export async function fetchAndRenderNabhikFile(file,fileDescription, container) {
  try {
      if (file.format.toLowerCase() === "png" || file.format.toLowerCase() === "html" || file.format.toLowerCase() === "jpeg") {
          const datadiv = document.createElement("div");
          datadiv.id = file.name;
          datadiv.classList.add("overflow-x-auto", "w-full", "border-gray-800", "scrollbar");
          container.appendChild(datadiv);
          renderNabhikFiles(file,fileDescription, datadiv, datadiv.id);
          container.scrollTop = container.scrollHeight;

      } else if (file.format.toLowerCase() === "json") {
      ...
      } else if (file.format.toLowerCase() === "csv") {
          const decoder = new TextDecoder("utf-8");
          const csvData = atob(file.data);
          ...
          const datadiv = document.createElement("div");
          datadiv.id = generateUUID();
          datadiv.classList.add("overflow-x-auto", "border-zinc-500", "text-xs", "scrollbar","text-gray-100");
          container.appendChild(datadiv);
          const tableDivId = generateUUID();
          let table = addTable(datadiv, tableDivId);
          table.classList.add("stripe","row-border","compact");
          datadiv.appendChild(table);
          addTableHeader(table, headers);
          renderDatatable(tableDivId, csvRows, headers, null,5,"400px");
          container.scrollTop = container.scrollHeight;
      } else {
          container.innerHTML = `<p>Unsupported file type: ${file.format}</p>`;
      }
...
export function addChatStateBox(cont, contType, canvas) {
  let fCon = "";
  if (contType === "JSON") {
      try {

          var jsonCon = JSON.parse(cont);
          fCon = jsonCon.key + " : " + jsonCon.value;
      } catch (error) {
          fCon = cont;
      }
  } else {
  ...
  const box = document.createElement('div');
  box.classList.add("text-xs",  "p-1", "break-words");
  box.innerHTML = `${fCon}<span>.</span><span>.</span><span>.</span>`
  canvas.appendChild(box);
  canvas.scrollTop = canvas.scrollHeight;
}
