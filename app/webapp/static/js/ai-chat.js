...
export async function aiInterfaceAction(apiUrl,
  ...
  if (input === "" && promptInput === "") {
    showErrorMessage("User Input Error", "Please provide a valid input for the AI model.");
    return;
  }
  ...
  if (input) {
    if (input !== "") {
      ...
      showAlert(AlertError, "User Input Error", "Please provide a valid input for the AI model.");
      return;
    }
  }
  if (temperature) {
  ...
  try {
    if (mode === 'CHAT_MODE') {
      return chatServer(payload, chatAPIUrl, myHeaders, resultDiv, rReqId, pRReqId, currentChatId);
    } else {
      if (isStream) {
        payload.stream = true;
        return streamServer(payload, apiUrl, myHeaders, resultDiv, rReqId, pRReqId);
      }
...
async function chatServer(payload, apiUrl, myHeaders, resultDiv, messageDivId, pDivId, currentChatId) {
  try {
    ...
    contentpre.classList.add('text-sm', 'p-2', 'rounded-lg', 'mt-2', "break-words", "whitespace-pre-wrap")
...
async function fetchServer(payload, apiUrl, myHeaders, resultDiv, messageDivId, pDivId) {
  ...
  try {
    ...
    const pDiv = document.getElementById(pDivId);
    let contentpre = document.createElement('div')
    contentpre.classList.add('text-gray-100', 'text-sm', 'p-2', 'rounded-lg', 'mt-2', "break-words", "whitespace-pre-wrap")
    document.getElementById('loader-' + pDivId).style.display = "none";
    ...
    if (
      ...
      jsonResponse.choices.forEach(val => {
        if (val.messages.content !== "") {
          console.log(val.messages.content,"------------------------------------->>>>>>>>>>>>>>>>>>>>>>");
          contentpre.innerHTML += renderMarkdown(decodeHTMLEntities(val.messages.content));
    ...
    if (chatObj.messages.length > 0) {
      chatObj.messages.forEach((msg) => {
        ...
        if (msg.role.toLowerCase() === "user") {
          ...
          repContDiv.classList.add("text-left", "rounded-xl", "bg-zinc-900 text-gray-100");
...
async function selectPrompt(promptId, tokenKey, apiUrl) {
  ...
  const response = await fetch(apiUrl, getRequestOptions(tokenKey, "GET", null));

  // Process the response
  if (!response.ok) {
    showErrorMessage("Prompt Selection Error", "Failed to select the prompt. Please try again or continue without selecting a prompt.");
    return
  }
  ...
  result.output.forEach((output) => {
    if (output.spec.variables.length > 0) {
      ...
      document.getElementById("promptInput").value = JSON.stringify(variablesMap, null, 2);
